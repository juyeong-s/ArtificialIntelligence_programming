...
x_tip = np.arange(0, 31, 1)  # 팀의 범위

# 소속함수 정의
qual_poor = fuzz.trimf(x_qual, [0, 0, 13])    # 음식 품질 . 사다리꼴 함수
qual_amazing = fuzz.trimf(x_qual, [7, 9, 10])   # 맛있다

serv_poor = fuzz.trimf(x_serv, [0, 0, 5])    # 서비스 만족도. 가우시안 소속함수
serv_acceptable = fuzz.trimf(x_serv, [0, 5, 10])
serv_amazing = fuzz.trimf(x_serv, [5, 10, 10])
...
def compute_tip_amount(qual_val, serv_val):
    ...
    # 각 규칙의 추론결과 결합
    aggregated = np.fmax(tip_activation_low, np.fmax(
        tip_activation_medium, tip_activation_high))
